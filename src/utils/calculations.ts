...
export const calculatePortfolioMetrics = (
  ...
  const riskScore = assets.reduce((sum, asset) => {
    const weight = asset.currentValue / totalValue;
    const riskValue =
      asset.riskLevel === 'low' ? 1 : asset.riskLevel === 'medium' ? 2 : 3;
  ...
  const maxAllocation = Math.max(
    ...assets.map((asset) => asset.currentValue / totalValue)
  );
  const diversificationScore = Math.min(
    100,
    typeCount * 20 + (1 - maxAllocation) * 50
  );
...
export const calculateVolatility = (assets: Asset[]): number => {
  ...
  const weightedVolatility = assets.reduce((sum, asset) => {
    ...
    const assetVolatility =
      asset.riskLevel === 'low' ? 5 : asset.riskLevel === 'medium' ? 15 : 25;
...
export const calculateRebalancingActions = (
  ...
  const totalValue = currentAssets.reduce(
  ...
  return currentAssets.map((asset) => {
    ...
    return {
      ...
      action:
        Math.abs(difference) < totalValue * 0.01
          ? 'hold'
          : difference > 0
            ? 'buy'
            : 'sell',
...
export const calculatePACProjection = (
  ...
  const monthsPerContribution =
    pacPlan.frequency === 'monthly'
      ? 1
      : pacPlan.frequency === 'quarterly'
        ? 3
        : pacPlan.frequency === 'biannual'
          ? 6
          : 12;
  ...
  const weightedReturn = Object.entries(pacPlan.targetAllocations).reduce(
...
export const getContributionFrequencyMultiplier = (
  frequency: PACPlan['frequency']
): number => {
  switch (frequency) {
    case 'monthly':
      return 12;
