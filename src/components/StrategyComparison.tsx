...
export const StrategyComparison: React.FC<StrategyComparisonProps> = ({
  ...
  const comparisonData = strategies.map((strategy) => ({
    name:
      strategy.name.length > 15
        ? strategy.name.substring(0, 15) + '...'
        : strategy.name,
        ...
        <div className="bg-white p-4 border border-gray-200 rounded-lg shadow-lg">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} className="text-sm" style={{ color: entry.color }}>
              {entry.dataKey === 'rendimento' &&
                `Rendimento: ${formatPercentage(entry.value)}`}
              {entry.dataKey === 'rischio' &&
                `Rischio: ${entry.value.toFixed(1)}/5`}
              {entry.dataKey === 'sharpe' &&
                `Sharpe: ${entry.value.toFixed(2)}`}
              {entry.dataKey === 'volatilita' &&
                `Volatilità: ${formatPercentage(entry.value)}`}
            </p>
          ))}
        </div>
      );
